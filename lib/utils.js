...
const JSIdentifierName = /^[\p{ID_Start}$_][\p{ID_Continue}$\u200C\u200D]*$/u;
function propertyName(name) {
  // All Web IDL identifiers are valid JavaScript PropertyNames, other than those with '-'.
  const isJSIdentifier = JSIdentifierName.test(name);
  if (isJSIdentifier) {
    return name;
  }
...
function stringifyPropertyAccess(prop) {
  if (typeof prop === "symbol") {
    return `[${symbolName(prop)}]`;
  }
  return !JSIdentifierName.test(prop) ? `[${JSON.stringify(prop)}]` : `.${prop}`;
}
...
function stringifyPropertyName(prop) {
  return typeof prop === "symbol" ? symbolName(prop) : JSON.stringify(prop);
}
