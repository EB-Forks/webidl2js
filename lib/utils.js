...
function getCallWithArgs(callWith, isStatic = false) {
  if (callWith.name !== "CallWith") {
    throw new Error("'utils.getCallWithArgs' can only be used for the [CallWith] extended attribute");
  }

  const { rhs } = callWith;
  if (!rhs || (rhs.type !== "identifier-list" && rhs.type !== "identifier")) {
    throw new Error("[CallWith] must take an identifier or an identifier list");
  }

  const result = [];
  const value = rhs.type === "identifier-list" ? rhs.value.map(identifier => identifier.value) : rhs.value;

  for (const callWithArg of CALL_WITH_ARGS) {
    if (callWithArg.staticOnly && !isStatic) {
      continue;
    }

    if ((rhs.type === "identifier" && value === callWithArg.name) ||
      (rhs.type === "identifier-list" && value.includes(callWithArg.name))) {
      result.push(callWithArg.name);
    }
  }
