...
class Interface {
  ...
  * allMembers(seen = new Set([this.name]), root = this.name) {
  ...
  addConstructor() {
    ...
    this._constructor = { args: argNames, body };
  }
  ...
  generateOffInstanceMethods() {
    ...
    addOne("constructor", this._constructor.args, this._constructor.body);
  ...
  generateOnInstance() {
    ...
    if (hasOwnProps) {
      this.str += `
        utils.initOwnMethodPrototypes(wrapper, globalObject.Function.prototype);
      `;
    }
  }
  ...
  generateInstall() {
    ...
    this.str += `class ${name} extends globalObject.${idl.inheritance || "Object"} {`;
    this.generateOffInstanceMethods();
    this.str += "}";
    if (!idl.inheritance) {
      this.str += `Object.setPrototypeOf(${name}, globalObject.Function.prototype);`;
    }

    this.generateOffInstanceAfterClass();
    if (this._outputStaticMethods.size > 0) {
      this.str += `\
        utils.initOwnMethodPrototypes(${name}, globalObject.Function.prototype);
      `;
    }

    if (this._hasProtoMethods) {
      this.str += `\
        utils.initOwnMethodPrototypes(${name}.prototype, globalObject.Function.prototype);
      `;
    }
