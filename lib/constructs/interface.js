  ...
  * allMembers(seen = new Set([this.name]), root = this.name) {
  ...
  generateLegacyProxyHandler() {
    ...
    const supportsPropertyIndex = (O, index, indexedValue) => {
      ...
      if (unsupportedValue) {
        const func = this.indexedGetter.name ?
          utils.stringifyPropertyAccess(this.indexedGetter.name) :
          "[utils.indexedGet]";
    ...
    const supportsPropertyName = (O, P, namedValue) => {
      ...
      if (unsupportedValue) {
        const func = this.namedGetter.name ?
          utils.stringifyPropertyAccess(this.namedGetter.name) :
          "[utils.namedGet]";
    ...
    const invokeIndexedSetter = (O, P, V) => {
      ...
      if (!this.indexedSetter.name) {
        ...
        invocation = `
          ${O}[implSymbol]${utils.stringifyPropertyAccess(this.indexedSetter.name)}(index, indexedValue);
        `;
      }
    ...
    const invokeNamedSetter = (O, P, V) => {
      ...
      if (!this.namedSetter.name) {
        ...
        invocation = `
          ${O}[implSymbol]${utils.stringifyPropertyAccess(this.namedSetter.name)}(${P}, namedValue);
        `;
      }
    ...
    if (this.supportsNamedProperties && !this.isGlobal) {
      ...
      if (!hasNamedDeleter) {
        ...
        let invocation;
        const func = this.namedDeleter.name ?
          utils.stringifyPropertyAccess(this.namedDeleter.name) :
          "[utils.namedDelete]";
