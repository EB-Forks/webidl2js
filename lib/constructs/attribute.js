...
class Attribute {
  ...
  generate() {
    ...
    `;
    let getterBody = `return utils.tryWrapperForImpl(esValue[implSymbol]${utils.stringifyPropertyAccess(this.idl.name)});`;
    let setterBody = `esValue[implSymbol]${utils.stringifyPropertyAccess(this.idl.name)} = V;`;
    if (conversions[this.idl.idlType.idlType]) {
      getterBody = `return esValue[implSymbol]${utils.stringifyPropertyAccess(this.idl.name)};`;
    }
    ...
    if (this.static) {
      brandCheck = "";
      getterBody = `return Impl.implementation${utils.stringifyPropertyAccess(this.idl.name)};`;
      setterBody = `Impl.implementation${utils.stringifyPropertyAccess(this.idl.name)} = V;`;
    ...
    if (!this.idl.readonly) {
      ...
      if (replaceable) {
        ...
        setterBody = `
          const Q = esValue${utils.stringifyPropertyAccess(this.idl.name)};
          if (!utils.isObject(Q)) {
