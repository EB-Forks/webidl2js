...
class Operation {
  ...
  getCallWith() {
    ...
    const firstCallWith = utils.getExtAttr(idls[0].extAttrs, "CallWith");
    const hasCallWith = Boolean(firstCallWith);

    if (hasCallWith && (!firstCallWith.rhs ||
      (firstCallWith.rhs.type !== "identifier" && firstCallWith.rhs.type !== "identifier-list"))
    ) {
      throw new Error(
        `[CallWith] must take an identifier or an identifier list: operation "${this.name}" on ${this.interface.name}`
      );
    }

    for (let i = 1; i < idls.length; i++) {
      const currentCallWith = utils.getExtAttr(idls[i].extAttrs, "CallWith");
      if (hasCallWith !== Boolean(currentCallWith)) {
        throw new Error(
          `[CallWith] is not applied uniformly to operation "${this.name}" on ${this.interface.name}`
        );
      }
    }
  ...
  generate() {
    ...
    const callWith = this.getCallWith();
    ...
    const args = [parameterConversions.hasArgs ? "...args" : ""];
    requires.merge(parameterConversions.requires);
    str += parameterConversions.body;

    if (callWith) {
      args.unshift(...utils.getCallWithArgs(callWith, this.static));
    }

    let invocation;
    if (overloads.every(overload => conversions[overload.operation.idlType.idlType])) {
      invocation = `
        return ${callOn}.${implFunc}(${args.join(", ")});
      `;
    } else {
      invocation = `
        return utils.tryWrapperForImpl(${callOn}.${implFunc}(${args.join(", ")}));
      `;
    }
