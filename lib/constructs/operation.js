...
class Operation {
  ...
  generate() {
    ...
    if (overloads.every(overload => conversions[overload.operation.idlType.idlType])) {
      invocation = `
        return ${callOn}${utils.stringifyPropertyAccess(implFunc)}(${utils.formatArgs(args)});
      `;
    } else {
      invocation = `
        return utils.tryWrapperForImpl(${callOn}${utils.stringifyPropertyAccess(implFunc)}(${utils.formatArgs(args)}));
      `;
    }
